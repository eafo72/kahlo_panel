...
const ReservacionesCancelarPage = () => {
    ...
    const handleReservationCancel = async () => {
        try {
            ...
            const checkResponse = await clienteAxios.post('/venta/cancelar', {
            ...
            if (checkResponse.data.error) {
                throw new Error(checkResponse.data.msg || 'Error al cancelar la reservación');
            }
            ...
            console.error('Error al procesar la cancelación de la reservación:', error);
            const errorMessage = error.response?.data?.msg || error.message || 'Error al procesar la cancelación de la reservación';
                                                        ...
                                                        onClick={() => {
                                                            toast.info(
                                                                <div className="text-center">
                                                                    <div className="mb-4">
                                                                        <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-200">¿Estás seguro?</h4>
                                                                        <p className="text-slate-600 dark:text-slate-300 mt-2">Esta acción cancelará la reservación y no se podrá deshacer.</p>
                                                                    </div>
                                                                ...
                                                                </div>,
                                                                {
                                                                    ...
                                                                    toastId: 'cancel-confirmation'
                                                                }
                                                            );
                                                        }}
                                                        disabled={isPaymentInProgress}
                                                    >
                                                        {isPaymentInProgress ? 'Procesando...' : 'Cancelar Reservación'}
                                                    </button>
                                                </>
                                            )}
                                        </div>
...
const Step1 = ({ fechaAnterior, contactInfo = {}, ticketQuantities = { entradaTipoA: 0, entradaTipoB: 0, entradaTipoC: 0 }, reservationNumber = '' }) => {
                    ...
                    <div className="flex">
                        ...
                        <div className="ml-3">
                            <div className="mt-2 text-sm text-blue-700 dark:text-blue-300">
                                ...
                                <p className="mb-2">Fecha y hora actual de la reserva: {formatearFecha(fechaAnterior)} a las {formatearHora(fechaAnterior)}</p>
...
export default ReservacionesCancelarPage;
