...
const Canceladas = () => {
  ...
  const getHistory = async (tipoUsuario, idempresa, idusuario) => {
    ...
    if (parseInt(tipoUsuario) == 1) {
      try {
        const res = await clienteAxios.get(`/admin/viaje-tour/viaje-Tours-canceladas`);
      ...
      try {
        const res = await clienteAxios.get(`/admin/viaje-tour/historialByEmpresa-canceladas/${idempresa}/admin/${idusuario}`);
        console.log(res.data);
        setDatos(res.data);
        setDatosOriginales(res.data); // 👈 guardamos copia original
      } catch (error) {
    ...
    <>
      <Card noborder>
        <div className="md:flex justify-between items-center mb-6">
          <h4 className="card-title">Ventas Canceladas</h4>
          {/*
