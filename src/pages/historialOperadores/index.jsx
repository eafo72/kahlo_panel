...
const HistorialOperadores = () => {
  const COLUMNS = [
    ...
    {
      Header: "Nombre cliente asignado",
      accessor: "nombre_cliente_asignado",
      Cell: (row) => {
        return <span>{row?.cell?.value}</span>;
      },
    },
  ...
  const getHistory = async (tipoUsuario, idempresa, idusuario) => {
    ...
    if (parseInt(tipoUsuario) == 1) {
      try {
        const res = await clienteAxios.get(`/admin/viaje-tour/viaje-Tours-Operadores`);
      ...
      try {
        const res = await clienteAxios.get(`/admin/viaje-tour/historialByEmpresaOperadores/${idempresa}/admin/${idusuario}`);
        console.log(res.data);
        setDatos(res.data);
        setDatosOriginales(res.data); // 👈 guardamos copia original
      } catch (error) {
    ...
    <>
      <Card noborder>
        <div className="md:flex justify-between items-center mb-6">
          <h4 className="card-title">Historial Operadores</h4>
          {/*
...
export default HistorialOperadores;
